...
void lambda_test() {
	...
	{
		using falcon::lambda::var;
		std::stringstream ss;
		auto f = var(ss) << '[' << phr::_1 << "]\n";
		f(5);
		f("plop");
		f(8);
	...
	{
    using falcon::lambda::func;
		{
			using namespace std::placeholders;
			auto f = func(std::bind(falcon::plus<>(), _1, _2));
			auto lambda_f2 = -f;
		...
		CHECK_EQUAL_VALUE(16, static_cast<int>((phr::_2 + falcon::lambda::func(lambda_f2))(4,6)));
	}

	{
		std::stringstream ss;
		std::wstringstream wss;
		(ss << phr::_1 << std::endl)(4);
		(wss << phr::_1 << std::endl)(4);
		(ss << phr::_1 << std::endl)(4);
		(wss << phr::_1 << std::endl)(4);
...

	{
		...
		{
			std::stringstream ss;
			auto p3 = var(ss) << _1 << std::endl;
			p3(56);
			CHECK_EQUAL_VALUE(ss.str(), "56\n");
		}
		{
			std::wstringstream wss;
			auto p3 = var(wss) << _1 << std::endl;
			p3(56);
			CHECK(wss.str() == L"56\n");
		}
	}
