...
namespace falcon {
	template<template<class CPP_PACK> class _TemplateFunctor, template<class CPP_PACK> class _Template, std::size_t _MaxDepth = -1u, bool _ErrFirstCallFunctorIsFalse = false>
	struct recursive_template
	{
		...
		class type_wrapper
		{
			...
			struct __impl_recursive<true, _Depth, parameter_pack<_U CPP_USE_OPTIONAL_PACK(_Args)> >
			{
				...
				typedef __impl_recursive<!!std::size_t(_Depth-1) && _TemplateFunctor<_U CPP_USE_OPTIONAL_PACK(_Args)>::value, std::size_t(_Depth-1),  parameter_pack<__first_type CPP_USE_OPTIONAL_PACK(_Args)> > __type;
		...
		class value_wrapper
		{
			...
			struct __impl_recursive<true, _Depth, parameter_pack<_U CPP_USE_OPTIONAL_PACK(_Args)> >
			{
				...
				static const __FALCON_TYPEOF_INIT_WRAP_EXPR(value, (__impl_recursive<_Depth-1 && _TemplateFunctor<_U CPP_USE_OPTIONAL_PACK(_Args)>::value, std::size_t(_Depth-1), parameter_pack<__type CPP_USE_OPTIONAL_PACK(_Args)> >::value));
			};
			template<std::size_t _Depth, typename _U CPP_OPTIONAL_PACK(_Args)>
			struct __impl_recursive<false, _Depth, parameter_pack<_U CPP_USE_OPTIONAL_PACK(_Args)> >
			{
